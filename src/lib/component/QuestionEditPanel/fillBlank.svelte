 ...
 * @LastEditors: LZX
 * @LastEditTime: 2025-07-08 14:54:45
 * @FilePath: \tutorial-platform-fe\src\lib\component\QuestionEditPanel\fillBlank.svelte
 * @Description: 题目编辑面板
 * @
-->
<script>
    ...
    import {BlankItem, CustomTextStyle} from "$lib/rich_text_extension/BlankItem.js";
    import BubbleMessageComponet from "$lib/component/BubbleMessageToast.svelte";
    import {Fragment} from 'prosemirror-model' // 确保已安装 prosemirror-model
    ...
    import {questionLimit} from "./questionConfig";
        ...
        if (title_editor && !editorInitialized) {
            ...
            const init = async () => {
                ...
                try {
                    ...
                    console.log("文档总长度:", piptap_editor.tiptapEditor.state.doc.content.size); // 35
     ...
     * @type {number[][]}
     */
    let question_answers_editors_warning = $state([[0, 0, 0, 0, 0, 0, 0, 0]]);
    ...
    let isSetupBlank = true

    // 变量注释
    /**
     * @typedef {import('@3min/cst-tiptap/dist/types').TiptapEditor} TiptapEditor
     * @type {TiptapEditor}
     * */
    function setupBlankRenumbering(editor) {
        ...
        function getAllBlanks() {
            let blanks = []
            const nodesToReplace = []; // 存储需要替换的节点信息
            editor.state.doc.descendants((node, pos) => {
                if (node.type.name === 'text' && node.marks && node.marks.length > 0 && node.marks[0].attrs.class && node.marks[0].attrs.class === 'blank-item') {
                    // 创建 BlankItem 节点
                    nodesToReplace.push({
                        pos,
                        node: {...node, text: node.text, nodeSize: node.nodeSize},
                        mark: node.marks[0], // 存储 mark 信息
                    });
                }
            })
            ...
            if (nodesToReplace.length > 0) {
                ...
                let posDelta = 0;
                nodesToReplace.forEach(({pos, node, mark}) => {
                    ...
                    // 删除代替换节点
                    tr.delete(pos + posDelta, pos + posDelta + node.nodeSize);

                    // 插入新节点
                    tr.insert(pos + posDelta, blankItemNode);
                    posDelta += blankItemNode.nodeSize
                    posDelta -= node.nodeSize

                });
                editor.view.dispatch(tr);
            }

            // 遍历文档（因为文档已修改），获取所有 blankItem 节点
            editor.state.doc.descendants((node, pos) => {
    ...
    const onEdiConfirm = () => {
        ...
        for (let i = 0; i < question_answers.length; i++) {
            ...
            if (question_score > questionLimit.scoreLimit) {
                for (
                    let i = 0;
                    i < question_answers_editors_warning.length;
                    i++
                ) {
            ...
            if (item.grading_rule.trim().length === 0) {
                ...
                item.grading_rule.length >
                questionLimit.gradingRuleCharacterLenLimit
            ) {
            ...
            for (
                let index = 0;
                index < item.alternative_answers.length;
                index++
            ) {
                if (item.alternative_answers[index].trim().length === 0) {
                    question_answers_editors_warning[item.index - 1][
                    index + 3
                        ] = 1;
                    success_confirm_flag = false;
                    break;
                } else if (
                    item.alternative_answers[index].length >
                    questionLimit.fillBlankAnswerCharacterLenLimit
                ) {
                    question_answers_editors_warning[item.index - 1][
                    index + 3
        ...
        if (question_answers.length !== blankNodes.size) {
            bubble_message_componet?.show(
                "error",
                "请检查填空项数量是否与答案数量一致",
            );
                    ...
                    return (
                        ...
                        (value.alternative_answers === undefined ||
                            (value.alternative_answers?.length ===
                                new_answer.alternative_answers?.length &&
     ...
     * @param {number} index
     */
    const onClickAnswerDeleteBtn = (e, index) => {
        e.stopPropagation();
    ...
    const onDeleteAlternaviteAnswer = (answer, index) => {
        if (
            ...
            answer.alternative_answers.length > 0
        ) {
            const new_main_answer =
                question_answers[index].alternative_answers.shift();
    ...
    const onAnswerScoreChange = (e, index, answer) => {
        if (
            ...
            if (
                typeof per_answer_score === "number" &&
                limited_value !== per_answer_score
            ) {
                per_answer_score = "";
            }
        ...
        if (
            ...
            const limited_value =
                value > 0
                    ? value <=
                    Math.floor(
                        questionLimit.scoreLimit / question_answers.length,
                    )
                        ? value
                        : Math.floor(
      ...
      >
        ...
        >
          <div class="labelText">
                        ...
                        ><span style="color:#D9001B">*</span>题目:</span
                        >
          </div>
        ...
        <div class="container difficultyOptionsContainer">
          <div class="labelText">
                        <span style="font-size: 14px;"
                        ><span style="color:#D9001B">*</span>难度:</span
                        >
          </div>
        ...
        >
          <div class="labelText">
            <span style="font-size: 14px;">解析:</span>
          </div>
          <div class="content richTextEditor">
            <div>
              {#if initialized}
...
{#snippet fillBlank()}
  <div class="container">
    <div class="labelText questionLabelText">
            <span style="font-size: 14px;"
            ><span style="color:#D9001B">*</span>答案:</span
            >
    </div>
    <div class="content fillBlankAnswerContent">
      {#each question_answers as answer, index}
        <div
          class="fillBlankAnswerRepeaterElement {question_answers_editors_warning[
                        index
                    ][0] !== 0 ||
          ...
          <div class="fillBlankAnswerContainer">
            <div class="fillBlankAnswerExpandBtn">
              ...
              >
                ...
                <!--                  class="deleteBinBtn"-->
                <!--                  onclick={(e) =>-->
                <!--                                        onClickAnswerDeleteBtn(e, index)}-->
                <!--                >-->
                <!--                  <img-->
                <!--                    src={ICON.delete_bin}-->
                <!--                    alt="delete_bin"-->
            ...
            <div class="fillBlankAnswerContent">
              {#if expend_answer_area[index]}
                <div
                  class="fillBlankAnswerContentContainer {question_answers_editors_warning[
                                        index
                                    ][0] !== 0 ||
                                    question_answers_editors_warning[
                                        index
                                    ][4] !== 0 ||
                                    question_answers_editors_warning[
                                        index
                ...
                >
                  <div
                    class="fillBlankAnswerContentInputContainer"
                  >
                                        <span>
                    ...
                    <div>
                      ...
                      <button
                        ...
                        onclick={(e) =>
                                                    onDeleteAlternaviteAnswer(
                                                        answer,
                                                        index,
                                                    )}
                      >
                        <img
                          src={ICON.Xacross_grey}
                          alt="Xacross_grey"
                        />
                      </button>
                      <button
                        class="alternativeAnswerBtn"
                        disabled={answer.alternative_answers &&
                                                answer.alternative_answers
                                                    .length >= 4
                                                    ? true
                                                    : false}
                        onclick={(e) =>
                                                    onAddAlternaviteAnswer(
                                                        index,
                                                    )}>追加答案
                      </button
                      >
                  ...
                  {#each answer.alternative_answers as alternative_answer, alternative_answer_index}
                    <div
                      class="fillBlankAnswerAlternativeContentInputContainer {question_answers_editors_warning[
                                                index
                    ...
                    >
                      <div>
                        ...
                        <button
                          class="deleteFillBlankAnswerBtn"
                          onclick={() => {
                                                        answer.alternative_answers.splice(
                                                            alternative_answer_index,
                                                            1,
                                                        );
                                                    }}
                        ...
                        <div
                          class="alternativeAnswerBtn isDiv"
                        >
                          占位
                        </div>
                      </div>
                  ...
                  >
                                        <span
                                          ...
                                          class="inputWarnText {question_answers_editors_warning[
                                                index
                                            ][0] !== 0
                                                ? 'warning'
                                                : ''}"
                ...
                <div
                  class="fillBlankAnswerScoreInputContainer {question_answers_editors_warning[
                                        index
                                    ][1] !== 0
                                        ? 'warning'
                                        : ''}"
                ...
                <div
                  class="fillBlankAnswerGradingRuleInputContainer {question_answers_editors_warning[
                                        index
                                    ][2] !== 0
                                        ? 'warning'
                                        : ''}"
      ...
      <div class="fillBlankAnswerControlContainer">
        ...
        <div>
          ...
          <div class="fillBlankTotalScore">
                        <span
                        ><span>共{question_answers.length}个填空项</span
                        ><span
                        >总分<span style="color: #0052D9;"
                        >{question_score}</span
                        >分</span
                        ></span
                        >
