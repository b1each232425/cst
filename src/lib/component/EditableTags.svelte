...
<script>
    ...
    const DEFAULT_COLORS = [
        "#40d5ff", "#59dcff", "#33c1e8", "#6adbff", 
        "#26caef", "#4dd6eb", "#6dcaf2", "#52c2ff", 
        "#7fd3f3", "#47d0db", "#5bc8f2", "#40c4e0",
        "#ffa040", "#ffb359", "#ffc26d", "#ffcf85",
        "#ff9eac", "#ffb3c0", "#ffc6d1", "#ffd9e0",
        "#d5a6ff", "#e0bfff", "#eed2ff", "#f2dcff",
        "#c6ff8c", "#d9ff99", "#e0ffb3", "#e6ffcc",
        "#72deff", "#4fd4d9", "#83def5"
    ];

    let {original_tags = DEFAULT_TAGS, max_tags_num = 6, max_tags_text_num = 20, input_type = "text", colors = DEFAULT_COLORS, onInputChange = (/** @type {string[]} */ tags)=>{ console.log(tags); }} = $props()
    ...
    //用于对标签数组进行处理后再显示
    $effect(()=>{handleOriginalTagsArray(tags)})
    ...
    export function getTags(){
        //拍摄快照，清除掉数组中的空元素后导出
        let export_tags = $state.snapshot(tags)
        let arr = export_tags
        arr = arr.filter(item => item !== "" && item !== null && item !== undefined);
        return arr;
    }
    ...
    function handleInput(event, index) {
        if (
            ...
            if (index === value_tags.length - 1 && 
                event.target.value.trim() !== "" && 
                value_tags.length < max_tags_num) {
...
<style lang="scss" scoped>
    ...
    .tag-input {
        ...
        color: #797979;
        transition: color 0.3s, border-bottom 0.3s; /* 添加过渡效果 */
