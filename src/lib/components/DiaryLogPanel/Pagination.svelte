 ...
 */
 -->
<script>
  import DropdownGray from '$lib/components/DiaryLogPanel/DropdownGray.svelte';
  ...
  //页码数组，用于存储将要显示出的页码
  let pages_array = $derived(calculatePagesArray(total_page_num, current_page_num, max_show_page_num));
    ...
    if (current === 1) {
      ...
      if (total - current > temp_page_num + 1) {
        ...
        if (total - current + temp_page_num + 1 < maxShow) {
          for (let i = current - (maxShow - (total - current + 2)); i <= total; i++) pages.push(i);
        } else {
...
{#snippet page_button(/** @type {any} */ number)}
  {#if number === current_page_num}
    ...
    >
  {:else}
    <button class="page-number-button" onclick={() => onPageChooseFunc(number)}>{number}</button>
  {/if}
{/snippet}
...
<div class="container">
  ...
  <input class="page_jump_input" type="number" oninput={handleInput} bind:this={search_input} min="1" />
</div>
