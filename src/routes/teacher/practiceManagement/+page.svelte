 ...
 * @LastEditTime: 2025-05-03 16:14:11
 -->
<script>
  ...
  import StudentSelectionPanel from "./components/StudentSelectionPanel.svelte";
  import {
    practice_data_list,
    ...
    current_page_store,
    page_size_store,
  } from "$lib/stores/practiceData.js";
  ...
  async function fetchPracticesFromServer(
    ...
    try {
      ...
      console.error("从服务器获取练习列表失败:", error);
    }
  }
  ...
  async function handleStudentSelectionConfirm(selected) {
    ...
    try {
      ...
      console.error("更新学生请求异常:", error);
      actionToast.show("error", "更新学生请求异常", "", 1000);
    }
  }
