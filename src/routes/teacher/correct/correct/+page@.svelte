    ...
    fetch(`/api/mark/details?${id_query}=${id}`)
      ...
      .then((res) => {
        if (!res.status) {
          ...
          if (!Array.isArray(question_sets)) throw new Error('试卷题目类型错误');
