...
import PaperSelectionPanel from '../_components/TestSelector.svelte';
...
describe('PaperSelectionPanel 组件测试', () => {
  ...
  describe('基础渲染测试', () => {
    ...
    it('当show为true时显示模态框', () => {
  render(PaperSelectionPanel, { show: true });
  expect(screen.getByText('选择试卷')).toBeInTheDocument();
});

    it('当show为false时隐藏模态框', () => {
  render(PaperSelectionPanel, { show: false });
  expect(screen.queryByText('选择试卷')).not.toBeInTheDocument();
});
    ...
    it('渲染表格头部', async () => {
      setup();
      await waitFor(() => {
        const headers = screen.getAllByText('试卷名称');
        const tags =screen.getAllByText('试卷标签')
        expect(headers[1]).toBeInTheDocument();
        expect(screen.getByText('组卷方式(练习类型)')).toBeInTheDocument();
        ...
        expect(screen.getByText('建议时长（分钟）')).toBeInTheDocument();
  ...
  describe('数据加载和显示测试', () => {
    ...
    it('正确显示组卷方式映射', async () => {
      ...
      await waitFor(() => {
        expect(screen.getAllByText('自定义组卷（经典巩固）')).toBeTruthy(); 
  ...
  describe('试卷选择功能测试', () => {
    ...
    it('显示已选择的试卷信息', async () => {
      const utils = setup({ selectedTestId: 1 });
...
beforeEach(() => {
  ...
  global.fetch.mockResolvedValueOnce({
    ...
    json: () => Promise.resolve({
      ...
      await waitFor(() => {
        expect(fetch).toHaveBeenCalledWith(
            "/api/paper?name=&tags=&page=1&pageSize=10&category=02",
            {
            method: 'GET',
            credentials: 'include',
          })
    ...
    it('API调用包含正确的查询参数', async () => {
      ...
      await waitFor(() => {
        ...
        expect(url).toContain('page=1');
        expect(url).toContain('pageSize=10');
        expect(url).toContain('category=02'); // 固定为考试类型
      });
    });
