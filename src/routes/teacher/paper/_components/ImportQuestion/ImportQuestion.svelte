...
<script>
    ...
    let selected_question_infos = $state([]);   // 已选题目数组
    ...
    function toggleSelection(id, checked) {
        if (checked) {
            ...
            if (QUESTION) {
                selected_question_infos.push(QUESTION);
            }
        } else {
            selected_question_infos = selected_question_infos.filter(question => question.ID !== id);
        }
        checkAllSelected();
    }
    ...
    function selectAllQuestions(checked) {
        if (checked) {
            // 只追加未存在的
            const EXISTING_IDS = selected_question_infos.map(question => question.ID);
            const TO_ADD_QUESTIONS = question_list.filter(question => !EXISTING_IDS.includes(question.ID));
            selected_question_infos = [...selected_question_infos, ...TO_ADD_QUESTIONS];
        } else {
            // 移除当前页的
            const CURRENT_PAGE_IDS = question_list.map(item => item.ID);
