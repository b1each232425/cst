<script>
  ...
  async function fetchAnalysisDataBySessionId(practiceId) {
    isLoaded = false;
    try {
      const response = await fetch(
        `/api/teacher/practice-analysis?practiceID=${practiceId}`,
        {
          method: "GET",
          credentials: "include",
        },
  ...
  onMount(async () => {
    await fetchAnalysisDataBySessionId(practiceData.id);
  });
