 ...
 * @LastEditTime: 2025-06-05 21:31:46
 * @FilePath: \tutorial-platform-fe\src\routes\teacher\questionBank\theory\+page.svelte
 * @Description: 
 * @
...
8""888P' `Y8bod8P' d888b    o888o  888bod8P'   "888" 
                                  ...
                                  o888o              
                                                      -->
<script>
    import BankCard from "$lib/component/BankCard.svelte";
    import { onMount } from "svelte";
    import { deepCopy } from "$lib/common/json_utils";
    import { formatTimestamp } from "$lib/common/time_utils";
    import { deleteBankData, getBankList, updateBankData } from "./api";
     ...
     * @description 批量删除题库
     */
    const onBatchDeleteBank = async () => {
        let to_be_delete_bank_list = bank_list
            .filter((item) => item.selected)
            .map((item) => {
                return item.id;
            });

        const res = await deleteBankData(to_be_delete_bank_list);
        if (res.status !== 0) {
            let show_message;
            switch (res.msg) {
            ...
            bubble_message_componet?.show(
                "error",
                "批量删除题库失败: " + show_message,
            );
            return;
        }
        origin_bank_list = origin_bank_list.filter(
            (item) => !to_be_delete_bank_list.includes(item.id),
        );
        bank_list = bank_list.filter(
            (item) => !to_be_delete_bank_list.includes(item.id),
        );

        bubble_message_componet?.show("success", "批量删除题库成功");
    };
    ...
    onMount(async () => {
        ...
        origin_bank_list = [];
        bank_list = [];
        for (let bank of res.data.bank_list) {
            bank_list.push({
                id: bank.id,
                name: bank.name,
                tags: bank.tags || [],
                create_time: `${formatTimestamp(bank.create_time, { show_time: false })}`,
                update_time: `${formatTimestamp(bank.update_time)}`,
                is_changed: false,
                is_hidden: false,
                selected: false,
            });
        ...
        preloadCode("/teacher/questionBank/theory/editBank");
        preloadCode("/teacher/questionBank/theory/newBank");

        bubble_message_componet?.show("info", "获取题库列表成功");

        // 页面刷新/关闭拦截处理
        // window.addEventListener("beforeunload", (event) => {
        //     if (had_add_beforeunload_event) {
        //         return;
        //     }

        //     had_add_beforeunload_event = true;

        //     interceptPageClose(event, "你有未保存的更改，是否要离开？");
        // });
    });
    ...
    function add_handle_func() {
        ...
        // window.location.href = `${window.location.pathname}/newBank`;
        goto(`${window.location.pathname}/newBank`);
    }
     ...
     * @param {number} index
     */
    async function onDeleteBankData(item, index) {
        ...
        bank_list = bank_list.filter((bank) => bank.id !== item.id);
        bubble_message_componet?.show("success", "删除题库成功");
    }
     ...
     * @param {number} index
     */
    async function onSaveBankData(item, index) {
        if (!item.is_changed) {
            return;
        }
        item.is_changed = false;

        let originBank = origin_bank_list.find((bank) => bank.id === item.id);
        if (originBank == null) {
            bubble_message_componet?.show("error", "题库数据不存在");
            return;
        }
        /**
         * @type {{
        ...
        if (item.name !== originBank.name) {
            update_fileds.name = item.name;
        }

        if (originBank.tags == null && item.tags == null) {
            // do nothing
        } else if (originBank.tags == null && item.tags != null) {
            update_fileds.tags = item.tags;
        } else if (originBank.tags != null && item.tags == null) {
            update_fileds.tags = [];
        } else if (originBank.tags?.length != item.tags?.length) {
            update_fileds.tags = item.tags;
        } else if (
            originBank.tags != undefined &&
            item.tags != undefined &&
            originBank.tags.length == item.tags.length
        ) {
            let origin_tags = originBank.tags;
            let tags_equal =
                item.tags.length === originBank.tags.length &&
                item.tags.every((value, index) => {
                    return value === origin_tags[index];
                });
            if (!tags_equal) {
                update_fileds.tags = item.tags;
            }
        ...
        origin_bank_list[index] = deepCopy(item);
        bubble_message_componet?.show("success", "保存题库成功");
    }
