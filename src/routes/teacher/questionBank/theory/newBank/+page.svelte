 ...
 * @LastEditTime: 2025-05-31 20:01:59
 * @FilePath: \tutorial-platform-fe\src\routes\teacher\questionBank\theory\newBank\+page.svelte
 * @Description: 理论题库新建页
 * @
-->
...
<script>
    ...
    import { compareBankMsg } from "../utils";
    import {
        AddOrUpdateQuestion,
        CopyToClipboard,
        onQuestinConfirm,
    ...
    onMount(async () => {
        ...
        request_lock = true;
        const res = await addNewBankData(bank_name, bank_tags, "00");
        request_lock = false;
        if (res.status !== 0) {
            let show_message;
            switch (res.msg) {
        ...
        bank_update_time = formatTimestamp(new Date().getTime());
        bank_create_time = formatTimestamp(new Date().getTime());
        bank_creator_id = res.data.creator;
        bank_access_mode = res.data.access_mode;

        let end = new Date().getTime();
        console.log("end loading ", end);
        ...
        window.addEventListener("beforeunload", (e) => {
            ...
            if (modifying_question) {
                navigator.sendBeacon(
                    `/api/questions/${modifying_question.id}/lock`,
                );
            }
        });

        bubble_message_componet?.show("success", "题库创建成功");
    });
    ...
    const onQuestionsImported = async (questions) => {
...
<div class="pageContainer">
    ...
    <div class="bankContent">
        ...
        <div class="questionListContainer">
            ...
            <div
                class="modal"
                style="display: {show_copy_panel ? 'flex' : 'none'};"
            >
    ...
    <SingleSelectEditPanel
        ...
        onConfirm={async (new_question_data) => {
            show_single_select_edit_panel = false;
            onEditPanelConFirm(new_question_data);
        }}
    ></SingleSelectEditPanel>
