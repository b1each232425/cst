 ...
 * @LastEditTime: 2025-04-10 17:19:09
 * @FilePath: \exam-fe\src\routes\teacher\questionBank\programming\+page.svelte
 * @Description: 
 * @
...
<script>
     ...
     * @type {Array<BankCardItemData>}
     */
    let bank_list = $state(deepCopy(origin_bank_list));
    ...
    $effect(() => {
        ...
        for(let item of bank_list){
            ...
            if(!item.name.includes(lower_case_search_input) && !item.tags?.some(tag => tag.includes(lower_case_search_input))){
                item.is_hidden = true;
                continue;
            }
...
<div class="question-bank-container">
    ...
    <div class="bank-container">
        <div class="bank-list">
            ...
            {#each bank_list as item}
                <div class="bank-card-container"
                    class:hidden={item.is_hidden}
                >
                    <BankCard
                        type="normal"
                        {icons}
    ...
    <div class="top-bar">
        ...
        <div class="operation-btns">
            <button style="background-color: #f36d78">
                <img src={icons.delete} alt="批量删除" />
                <span>批量删除</span>
            </button>

            <button style="background-color: #7787a2"
                onclick={() => {antiSelectAllHandleFunc()}}
            >
