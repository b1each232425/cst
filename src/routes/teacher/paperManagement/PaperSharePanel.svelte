<script>
     ...
     * @property {number} id - Paper ID
     * @property {string} name - Paper name
     * @property {string} access_mode - Paper access mode
     ...
     * @property {ShareUserInfo} creator - Paper creator info
     */

    let { isOpen = $bindable(), paper, actionToast } = $props();
    ...
    async function fetchSharedUsers() {
        ...
        errorMsg = "";
        try {
            const resp = await fetch(`/api/paper/${paper.id}/share`, {
                credentials: "include",
            });
            ...
            shareStatus = paper.access_mode || "00";
            UserID = result.data.user_id;
            isOwner = UserID === creator?.user_id;
            selectedUserIds = sharedUsers.map(user => user.user_id);
        } catch (error) {
            errorMsg = "获取共享信息失败:" + (error instanceof Error ? error.message : "未知错误");
        } finally {
            loading = false;
        }
    }
    ...
    async function searchUsers() {
        ...
        try {
            ...
            if (searchAccount.trim()) {
                params.append("account", searchAccount.trim());
            }
            if (searchName.trim()) {
            ...
            if (result && result.data) {
                userList = result.data.map((/** @type {Teacher} */ teacher) => ({
                    id: teacher.id,
    ...
    $effect(() => {
        if (isOpen && paper && is_initial_loaded) {
        ...
        <div class="panel-body">
            ...
            {#if shareStatus === "02"}
                ...
                <div class="user-select-container">
                    <div class="search-container" style="display: flex; gap: 12px;">
                        <SearchInput
                            purpose_text="账号"
                            place_holder="请输入账号"
                            onSearchFunc={handleAccountSearch}
                        />
                        <!-- 姓名搜索 
                        <SearchInput
                            purpose_text="姓名"
                            place_holder="请输入姓名"
                            onSearchFunc={handleNameSearch}
                        />
                        -->
                    </div>
                        ...
                        <table class="table" class:loading>
                            ...
                            <tbody>
                                {#each userList as user}
                                    <tr class={`user-row ${user.selected ? "selected" : ""}`}>
                                        <td>
                                            <input
                                                type="checkbox"
                                                class="custom-checkbox"
                                        ...
                                        <td>
                                            {#if sharedUsers.some(shared => shared.user_id === user.id)}
                                                <span class="status-tag">已共享</span>
                                            {/if}
                                        </td>
                                    </tr>
                                {/each}
