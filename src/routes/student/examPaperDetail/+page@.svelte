<script>
  ...
  import { formatTimestamp } from "$lib/common/time_utils.js";
  ...
  // 考试信息
  let exam_info = $state(page.data.exam_info);

  /**
   * 考试场次信息
   * @type {ExamineeSession[]}
   */
  let exam_sessions = $state(page.data.exam_sessions);

  /**
   * 获取考试信息
   * @type {string | null}
   */
  let exam_id = $state(page.data.exam_id);

  // 考生须知内容
  let exam_notes = $state(page.data.exam_notes);
  ...
  /**
   * 查看考试是否提交
   * @param {number}examinee_id
   */
  async function checkIfSubmitExam(examinee_id) {
    if (!examinee_id) {
      alert("examinee_id is null");
      return;
    }

    try {
      const response = await fetch(
        `/api/CheckIfSubmitExam?examinee_id=${encodeURIComponent(examinee_id)}`,
        { method: "GET", credentials: "include" }
      );

      if (!response.ok) {
        console.error("获取提交时间失败");
        throw new Error(`获取提交时间失败:${response.status}`);
      }

      const data = await response.json();
      console.log("data:", data);
      if (data.status !== 0) {
        throw new Error(`获取提交时间失败:${data.msg}`);
      } else {
        console.log(data);
        return data.data.EndTime;
      }
    } catch (e) {
      console.error("获取提交时间失败:", e);
      actionToast.show("error", "获取提交时间失败");
   ...
   * @param {number}examinee_id
   */
  async function checkIfHaveStartTime(examinee_id) {
    try {
      const response = await fetch(
        `/api/exam/startTime?examinee_id=${examinee_id}`
      );
      if (!response.ok) {
   ...
   */
  async function fetchExamStatus(exam_id, examinee_id) {
    try {
      const response = await fetch(
      ...
      actionToast.show("error", "获取考试状态失败");
    }
  }
  ...
  $effect(() => {
    ...
    if (exam_id && exam_sessions && exam_sessions.length > 0) {
      fetchExamStatus(exam_id, exam_sessions[current_session].ExamineeID);
    }
  });
  /**
   * 下载文件
  ...
  onMount(async () => {
    ...
    if (exam_id && exam_sessions && exam_sessions.length > 0) {
      await fetchExamStatus(exam_id, exam_sessions[current_session].ExamineeID);
    }
    if (!exam_id) {
