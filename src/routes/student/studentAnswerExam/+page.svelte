 ...
 * @LastEditTime: 2025-06-22 14:30:09
  ...
  import ExamInfoModal from "$lib/component/ExamInfoModal/ExamInfoModal.svelte";
  ...
  import { checkExamStatus } from "$lib/common/check_exam_status";
  ...
  //查看当前是否为预览模式
  let ifPreview =
    page.data.examinee_id === null || page.data.examinee_id === undefined
      ? true
      : false;
  ...
  //考试模式 00:固定时段 02:灵活时段
  let period_mode = $state(page.data.period_mode);
  ...
  //倒计时结束后的提交逻辑
  async function submitExamAfterCountdown() {
    try {
      const body_data = {
      ...
      const resp = await fetch("/api/student/exam/submit", {
        method: "POST",
        headers: {
  ...
  async function getProgress() {
    try {
      const response = await fetch(
        `/api/student/exam/progress?examinee_id=${examinee_id}`,
  ...
  async function saveAnswer(
    ...
    try {
      ...
      // 发送请求
      const response = await fetch("/api/student/exam/answer", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
    ...
    <div class="left-info" class:hide={!showLeftInfo}>
      ...
      <div class="box exam-info">
        <div class="exam-time-info">
          考试信息: <button class="button" onclick={openModal}>查看详情</button>
        </div>
      ...
      <div class="box preference-info">
        ...
        <BulmaSwitch  bind:is_full_examMode={is_full_examMode}></BulmaSwitch>
            ...
            {#if is_full_examMode}
              ...
              {#each examQuestions as question, index}
                ...
                <div class="question-mark-container" id={`question-${index}`}>
                  <Question {question} {index} {ifPreview} {saveAnswer} {query_url} editor_height=200px
                  ></Question>
