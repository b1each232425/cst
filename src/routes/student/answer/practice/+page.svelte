<script>
  ...
  function getQuestionGroups() {
    ...
    // 确保 question_groups_map 是 Map 类型（兼容 localStorage 数据）
    const groupsMap =
      question_groups_map instanceof Map ? question_groups_map : new Map(Object.entries(question_groups_map || {}));

    // 确保 exam_questions_map 是 Map 类型（兼容 localStorage 数据）
    const questionsMap =
    ...
    // 构建全局索引映射（依赖 examQuestions，需确保已初始化）
  ...
  function flattenExamQuestions() {
    ...
    // 确保 question_groups_map 是 Map 类型（如果是 localStorage 数据，手动转换）
    const groupsMap =
      question_groups_map instanceof Map ? question_groups_map : new Map(Object.entries(question_groups_map || {}));
    ...
    sortedGroups.forEach((groupInfo) => {
      const groupId = groupInfo.ID;
      // 确保 exam_questions_map 是 Map 类型（如果是 localStorage 数据，手动转换）
      const questionsMap =
        exam_questions_map instanceof Map ? exam_questions_map : new Map(Object.entries(exam_questions_map || {}));
